...
st.set_page_config(layout="centered", page_title="AI 透析患者营养与饮食分析")
...
你是一位经验丰富的透析和肾脏病营养师，同时也是一位医生、护士和患者的AI助手。你的任务是全面分析患者的检查结果和一个月的透析治疗数据，并提供详细的营养见解和个性化的饮食建议。你的分析和建议应该：
1. 基于最新的营养学和临床指南，如KDIGO指南和中国相关指南。
2. 针对患者的具体情况，提供个性化和可操作的营养建议。
3. 逐一分析所有关键营养数据点，包括正常和异常值，并特别注意月度数据的趋势和变化。
4. 解释每个发现的营养意义，并将患者的数据与标准目标值进行详细比较。
5. 提供具体的改善建议，包括饮食调整、营养补充、生活方式建议。
...
* 每个发现应包括数据点、营养意义和需要关注的原因。

3. 详细分析
* 对于每个方面（见下方列表），请：
  * 逐一指出异常或需要关注的数据点，并包含正常参考范围。
  * 解释这些发现的营养意义，以及可能的原因和影响。
  * 分析月度数据的趋势和变化，使用图表或表格展示（如适用）。
  * 将患者的数据与标准指南或目标值进行详细比较，引用具体的指南和目标范围。
  * 提供具体的改善建议，包括可采取的措施和预期目标。
* 需要分析的方面： a. 蛋白质摄入 b. 钙磷代谢 c. 钾和钠平衡 d. 体重管理 e. 维生素和矿物质 f. 水分摄入 g. 饮食与药物相互作用 h. 长期营养预后

4. 饮食建议和调整方案
* 使用Markdown表格列出具体的饮食建议。
* 包括饮食调整（食物种类、摄入量）、营养补充、生活方式建议。
...
* 包括至少一个中国指南和一个KDIGO指南，并注明具体章节或页码。

7. 患者指导
* 提供简单易懂的建议，帮助患者理解和遵循饮食建议。
* 使用清晰的语言和图表，确保患者能够轻松理解和执行。
"""


def generate_structured_prompt(lab_data, treatment_data):
    # Extract key indicators from a single patient's data
    hemoglobin_value = lab_data['血常规']['血红蛋白']
    albumin_value = lab_data['肝功能']['白蛋白']
    phosphorus_value = lab_data['电解质']['无机磷']
    calcium_value = lab_data['电解质']['钙']
    potassium_value = lab_data['电解质']['钾']

    prompt = f"""
请分析以下透析患者的检查结果和一个月的透析治疗数据，并提供详细的分析和建议。
患者基本信息：
* 姓名：{lab_data['患者信息']['姓名']}
...
关键指标：
| 血红蛋白 (g/L) | {hemoglobin_value:.2f} |
| 白蛋白 (g/L) | {albumin_value:.2f} |
| 磷 (mmol/L) | {phosphorus_value:.2f} |
| 钙 (mmol/L) | {calcium_value:.2f} |
| 钾 (mmol/L) | {potassium_value:.2f} |
...
* 涵盖所有关键数据点，包括正常和异常值。
* 遵循最新的营养和临床指南，并在比较时引用具体的标准和目标范围。
* 提供可操作的建议，便于医生、护士和患者快速理解和采取行动。
* 使用Markdown格式来呈现表格和图表，确保清晰易读。
"""
    return prompt

# Streamlit app
st.title("AI 透析患者营养与饮食分析")

st.markdown("""
本应用程序利用多种AI模型（GPT-4、Gemini Pro、Claude-3）来分析透析患者的营养和饮食数据。
它提供基于最新营养学指南的详细分析和个性化饮食建议，包括一个月的透析治疗数据和趋势图表。
""")
...
# Patient selection
st.header("选择患者")
selected_patient = st.selectbox("请选择要分析的患者", patients)
...
if selected_patient:
    ...
    if st.button('开始分析'):
        ...
        tab1, tab2, tab3 = st.tabs(["GPT-4o", "Gemini Pro", "Claude-3"])

        with tab1:
            st.subheader("GPT-4o 营养分析")
            with st.spinner('GPT-4o 分析中，请稍候...'):
                start_time_gpt = time.time()
                model_type, gpt_insights = generate_gpt_insights(prompt, "透析和肾脏病营养专家")
                end_time_gpt = time.time()
                st.info(f"GPT-4o 分析耗时: {end_time_gpt - start_time_gpt:.2f} 秒")
                st.markdown(gpt_insights)
                progress_bar.progress(1 * progress_step)
                st.download_button("下载分析报告", gpt_insights, file_name="GPT-4o 营养分析报告")
        with tab2:
            st.subheader("Gemini Pro 营养分析")
            with st.spinner('Gemini Pro 分析中，请稍候...'):    
                start_time_gemini = time.time()
                gemini_insights = generate_gemini_insights(prompt, "透析和肾脏病营养专家")
                ...
                st.download_button("下载分析报告", gemini_insights, file_name="Gemini Pro 营养分析报告")
        with tab3:
            st.subheader("Claude-3 营养分析")
            with st.spinner('Claude-3 分析中，请稍候...'):
                start_time_claude = time.time()
                claude_insights = generate_anthropic_insights(prompt, "透析和肾脏病营养专家")
                ...
                st.download_button("下载分析报告", claude_insights, file_name="Claude-3 营养分析报告")
else:
    st.info("请点击'开始营养分析'以启动分析。")

st.markdown("---")
st.markdown("© 2024 AI 透析患者营养与饮食分析应用. 版权所有。")